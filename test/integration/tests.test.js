...
const params = {
  accessToken:
    "59fceefbb829023353b4961933b699896e2e5d92078f5e752aaee8d7c2612dfc",
  space: "ezs1swce23xe"
};
const localeSpaceParams = {
  accessToken:
    "da1dc0e316213fe11e6139d3cd02f853b12da3f3fd0b4f146a1613a9cca277cd",
  space: "7dh3w86is8ls"
};
...
const previewParams = {
  ...
  accessToken:
    "fc9c8a0968c592bd7a0a5a9167d6fb6002dbbc3b8f900a75708ec269332d250a",
  space: "ezs1swce23xe"
};
...
test("Gets entries with inequality query", async () => {
  ...
  expect(response.items.filter(item => item.sys.id === "nyancat")).toHaveLength(
    0
  );
});
...
test("Gets entries with array equality query", async () => {
  ...
  expect(response.total).toBe(1);
  expect(
    response.items[0].fields.likes.filter(i => i === "lasagna")
  ).toHaveLength(1);
});
...
test("Gets entries with exclusion query", async () => {
  ...
  expect(
    response.items[0].fields.likes.filter(i => i === "rainbow")
  ).toHaveLength(0);
});
...
test("Gets entries with exists query", async () => {
  ...
  expect(response.items.map(item => item.fields.likes)).toHaveLength(
    response.total
  );
});
