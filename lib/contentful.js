...
export function createClient (params) {
  ...
  const config = {
    ...
    // accessTokenBySpaceId: {kaubyxzjeuta: 'CFPAT-LltPAvoGiiby7yL0BzBdkbH_Tu5yky-JxDPD_uTffOc'}
  }
  ...
  const contentfulApiBySpaceId = Object.entries(config.accessTokenBySpaceId).reduce((acc,[spaceId, accessToken]) => {
     const httpClient = createHttpClient(axios, { ...config, accessToken, space: spaceId })
  ...
  console.dir(contentfulApiBySpaceId, {depth: null})
  contentfulApiBySpaceId[config.space].extraApis = contentfulApiBySpaceId
