...
export default function createContentfulApi ({http, shouldLinksResolve}) {
  ...
  /**
   * Gets a collection of Entries even if they span over a page of results
   * @memberof ContentfulClientAPI
   * @param  {Object=} query - Object with search parameters. Check the <a href="https://www.contentful.com/developers/docs/javascript/tutorials/using-js-cda-sdk/#retrieving-entries-with-search-parameters">JS SDK tutorial</a> and the <a href="https://www.contentful.com/developers/docs/references/content-delivery-api/#/reference/search-parameters">REST API reference</a> for more details.
   * @param  {boolean=} query.resolveLinks - When true, links to other Entries or Assets are resolved. Default: true.
   * @return {Promise<Entities.EntryCollection>} Promise for a collection of Entries
   * @example
   * client.getAllEntries({content_type: 'contentTypeId'})
   * .then(entries => console.log(entries.items))
   */
  function getAllEntries (query = {}) {
    const resolveLinks = shouldLinksResolve(query)
    const resolveForAllLocales = (query.locale && query.locale === '*')
    function recursivelyGetAllEntries (query = {}, data) {
      return http.get('entries', createRequestConfig({query: query}))
      .then((response) => {
        if (data == null) {
          data = response.data
        ...
        if (data.total > response.data.skip + response.data.limit) {
          query.skip = response.data.skip + response.data.limit
          return recursivelyGetAllEntries(query, data)
        } else {
          return wrapEntryCollection(data, resolveLinks, resolveForAllLocales)
        }
      }, errorHandler)
    }
    return recursivelyGetAllEntries(query)
  }
