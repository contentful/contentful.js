...
import { addResourceLinksToIncludes } from './utils/resource-links'
...
export default function createContentfulApi ({ http, getGlobalOptions }) {
   ...
   * console.log(entry)
   */
  async function getEntry (id, query = {}, maxDepth = 1) {
    if (!id) {
      throw notFoundError(id)
    }

    if (maxDepth < 1 || maxDepth > 10) {
      throw new Error('maxDepth should be a number between 1 and 10')
    }

    try {
      // maxDepth 1 does not resolve any resource links of the entry.
      const response = await this.getEntries({ 'sys.id': id, ...query }, { currentDepth: 0, maxDepth })
   ...
   * console.log(response.items)
   */
  async function getEntries (query = {}, depth) {
    const { resolveLinks, removeUnresolved, resolveResourceLinks } = getGlobalOptions(query)
    ...
    if (depth.currentDepth < depth.maxDepth) {
      ...
      try {
        ...
        depth.currentDepth++
        const data = resolveResourceLinks ? await addResourceLinksToIncludes(response.data, this.extraApis, depth) : response.data
        return wrapEntryCollection(data, { resolveLinks, removeUnresolved })
      } catch (error) {
        errorHandler(error)
      }
    }
    return wrapEntryCollection({ items: [] }, { resolveLinks, removeUnresolved })
  }
