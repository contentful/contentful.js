...
export default function createContentfulApi ({ http, getGlobalOptions }) {
  ...
  async function getEntries (query = {}) {
    ...
    try {
      ...
      const dataWithResourceLinkIncludes = await addResourceLinksToIncludes(response.data, this.extraApis)

      return wrapEntryCollection(dataWithResourceLinkIncludes, { resolveLinks, removeUnresolved })
    } catch (error) {
      errorHandler(error)
    }
  }
