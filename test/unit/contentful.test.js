...
describe("contentful", () => {
  ...
  test("Throws if no accessToken is defined", () => {
    expect(() => createClient({ space: "spaceId" })).toThrow(
      /Expected parameter accessToken/
    );
  });

  test("Throws if no space is defined", () => {
    expect(() => createClient({ accessToken: "accessToken" })).toThrow(
  ...
  test("Initializes API with link resolution turned on by default", () => {
    ...
    expect(
      callConfig[0].getGlobalOptions({ resolveLinks: false }).resolveLinks
    ).toBeFalsy();
  });
