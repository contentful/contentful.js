...
export default function createContentfulApi ({ http, getGlobalOptions }) {
  ...
  async function getEntries (query = {}) {
    ...
    const isResourceLink = (field) => field && field.sys && field.sys.type === 'ResourceLink'

    const getResourceLinkFields = (data) => {
      const items = [].concat(...data.items, ...Object.values(data.includes).flat())
      console.log('fields', items.map(({ fields }) => Object.values(fields)))
      const resourceLinkFields = items.flatMap(({ fields }) => Object.values(fields).flat()).filter(isResourceLink)
      console.log(resourceLinkFields)
      return false
    }

    const parseResourceLink = (resourceLink) => {
      // For now just assume every resourceLink has type Contentful:Entry
      const [, spaceId, , entityId] = resourceLink.sys.urn.split('/')
      return { spaceId, entityId }
    }

    const groupBy = (key, objArray) => {
    ...
    const resolveResourceLinks = (data) => {
      ...
      const resourceLinkResponses = Object.entries(entitiesBySpace).map(([spaceId, entities]) => {
        const apiInstance = this.extraApis[spaceId];
        if (!apiInstance) {
          return { items: [] };
        }
        const entityIds = [...new Set(entities.map(({ entityId }) => entityId))];
        return apiInstance.getEntries({ 'sys.id': entityIds });
      })
      return Promise.all(resourceLinkResponses);
    }
    ...
    try {
      ...
      const resourceLinks = await resolveResourceLinks(response.data);
      console.log('RESOLURCE LINKS');
      console.dir(resourceLinks.data, { depth: 6 })
  ...
  // TODO: parse urn to get spaceId and call the extra client with the non default spaceId
  // Append the entries to includes
