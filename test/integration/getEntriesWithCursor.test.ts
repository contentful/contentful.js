  ...
  clients.forEach(({ type, client }) => {
    describe(`${type} client`, () => {
      ...
      test('should return [limit] number of items', async () => {
        const response = await client.getEntriesWithCursor({ limit: 3 })
      ...
      test('should support forward pagination', async () => {
        const firstPage = await client.getEntriesWithCursor({ limit: 2 })
      ...
      test('should support backward pagination', async () => {
        const firstPage = await client.getEntriesWithCursor({ limit: 2, order: ['sys.createdAt'] })
        const secondPage = await client.getEntriesWithCursor({
          limit: 2,
          pageNext: firstPage.pages.next,
          order: ['sys.createdAt'],
        })
        const result = await client.getEntriesWithCursor({
          limit: 2,
