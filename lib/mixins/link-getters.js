 ...
 */
export default function mixinLinkGetters (items, includes, errors = []) {
  ...
  function setFieldGettersForFields (fields) {
    ...
    fieldKeys.forEach((fieldKey) => {
      ...
      const unresolved = errors.find((error) => error.details.id === field.sys.id)
      if (unresolved) {
  ...
  function addGetterForLinkArray (listField, fieldKey, fields) {
    if (listField.length &&
      listField[0] !== null &&
      typeof listField[0] === 'object' &&
      listField[0].hasOwnProperty('sys') &&
      listField[0].sys.hasOwnProperty('type') &&
      listField[0].sys.type === 'Link') {
      Object.defineProperty(fields, fieldKey, {
