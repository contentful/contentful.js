...
export default function createContentfulApi ({ http, getGlobalOptions }) {
  ...
  async function getEntry (id, query = {}) {
    ...
    try {
      const response = await this.getEntries({ 'sys.id': id, ...query }, { currentDepth: 0, maxDepth: query.include === undefined ? 1 : query.include })
  ...
  async function getEntries (query = {}, depth) {
    ...
    try {
      ...
      const data = await addResourceLinksToIncludes(response.data, this.extraApis, depth)
