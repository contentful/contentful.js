...
export default function createContentfulApi ({ http, getGlobalOptions }) {
   ...
   * console.log(response.items)
   */
  async function getEntries (query = {}, options = { wrapEntryCollection: true }) {
    switchToEnvironment(http)
    ...
    try {
      ...
      const data = resolveLinks ? await addResourceLinksToIncludes(response.data, this.extraApis, query) : response.data
      return options.wrapEntryCollection ? wrapEntryCollection(data, { resolveLinks, removeUnresolved }) : data
    } catch (error) {
      errorHandler(error)
    }
  }
