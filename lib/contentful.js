 ...
 * @prop {Object=} params.additionalTokens - Space ID and Contentful CDA Access Token pairs for additional spaces
...
export function createClient (params) {
  ...
  function createContentfulApiForSpace (space, accessToken, environment) {
    ...
    const getGlobalOptions = createGlobalOptions({
      ...
      environmentBaseUrl: `${httpClient.defaults.baseURL}environments/${environment}`
    })
  ...
  const contentfulApiBySpaceId = Object.entries(config.additionalTokens).reduce((result, [spaceId, accessToken]) => {
    if (spaceId !== config.space) {
      result[spaceId] = createContentfulApiForSpace(spaceId, accessToken, 'master')
    }
    return result
  }, {})

  contentfulApiBySpaceId[config.space] = createContentfulApiForSpace(config.space, config.accessToken, config.environment)

  Object.values(contentfulApiBySpaceId).forEach(contentfulApi => { contentfulApi.additionalApis = contentfulApiBySpaceId })
